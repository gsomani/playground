...
var p,T,diff_p,diff_T,diff,coeff_x,coeff_y,factor_x,factor_y;
...
function boundary_T(size_x,size_y,temp_boundary){
	...
	for(i=1;i<=size_y;i++) {T[0][i]=0;T[size_x+1][i]=T[size_x][i];}
	for(i=1;i<=size_x;i++) {T[i][0]=T[i][1];T[i][size_y+1]=T[i][size_y];}
	}
}
...
function update_p(dt,dx,size_x,size_y,tau,epsilon,gamma,alpha,T_e,a)
{
	var x,y,laplace,phase,noise;
	var mul=new Array(3);
	mul[0]=dt/tau;mul[1]=(epsilon*epsilon)/(dx*dx);mul[2]=alpha/Math.PI;
	boundary_p(size_x,size_y);
	for(x=1;x<=size_x;x++)
		for(y=1;y<=size_y;y++)
	{
		phase=p[x][y];m=mul[2]*Math.atan(gamma*(T_e-T[x][y]));
		noise=a*(Math.random()-0.5);
		laplace=p[x+1][y]+p[x-1][y]+p[x][y+1]+p[x][y-1]-4*phase;
		diff_p[x][y]=mul[0]*(mul[1]*laplace+phase*(1-phase)*(phase-0.5+m+noise));
	}
	for(x=1;x<=size_x;x++)
		for(y=1;y<=size_y;y++)
...
function solidify(phaseField,Temperature,dt,dx,size_x,size_y,tau,epsilon,gamma,alpha,T_e,a,K,temp_boundary,iterations,timeSteps,gray)
{
	var t;
	for(t=0;t<timeSteps;t++){	
	update_p(dt,dx,size_x,size_y,tau,epsilon,gamma,alpha,T_e,a);
	update_temp(K,dt,dx,temp_boundary,size_x,size_y,iterations);
	}
	fill_rect(phaseField,Temperature,size_x,size_y,gray);
}

function run(size_x,size_y,timeSteps,timeGap,gray)
{	
	var initial_temp = +document.getElementById("T_i").value,temp_boundary = +document.getElementById("T_b").value,K = +document.getElementById("K").value,phase_profile = document.getElementById("phase_profile").value;
	var dt=0.0002,dx=0.03,tau=0.0003,epsilon=0.01,gamma=10,alpha=0.9,a=0.01,T_e=1,iterations=5;
	...
	ctx.fillText("Phasefield", 5+size_x/2, size_y+20);
	ctx.fillText("Temperature", 1.5*size_x+10, size_y+20);
	...
	setInterval(function(){solidify(phaseField,Temperature,dt,dx,size_x,size_y,tau,epsilon,gamma,alpha,T_e,a,K,temp_boundary,iterations,timeSteps,gray)},timeGap);
